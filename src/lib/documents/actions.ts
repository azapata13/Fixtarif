...
import { createBillOfLadingPdf, createPackingSlipPdf } from "@/lib/documents/pdf";
import { getShipmentForWorkspace } from "@/lib/shipments/queries";
import { createClient } from "@/lib/supabase/server";
import type { DocumentType, ExtractionStatus, ValidationStatus } from "@/lib/supabase/types";
...
export async function generateBillOfLadingDraft(locale: Locale, formData: FormData) {
  const { workspace, membership, user } = await getCurrentWorkspace();
  ...
  if (shipment.status !== "ready") {
    redirect(`/${locale}/shipments/${shipmentId}?message=${encodeURIComponent("Marquez l'expédition prête avant de générer un BOL.")}`);
  }
  ...
  const documentType: DocumentType = "bol";
  const pdf = createBillOfLadingPdf({
    bolNumber: transport?.bol_number ?? null,
    carrierName: carrier?.name ?? null,
    contactEmail: contact?.email ?? null,
    contactName: contact?.name ?? null,
    contactPhone: contact?.phone ?? null,
  ...
  const supabase = await createClient();
  const storagePath = `${workspace.id}/${shipment.id}/${crypto.randomUUID()}-bol-draft.pdf`;
  const { error: uploadError } = await supabase.storage.from(GENERATED_BUCKET).upload(storagePath, pdf, {
    cacheControl: "3600",
    contentType: "application/pdf",
    upsert: false,
  ...
  if (uploadError) {
    logServerError({ action: "generate_bol_storage", error: uploadError });
    redirect(`/${locale}/shipments/${shipmentId}?message=${encodeURIComponent(genericActionError(locale))}`);
  }
  ...
  if (documentError) {
    await supabase.storage.from(GENERATED_BUCKET).remove([storagePath]);
    logServerError({ action: "generate_bol_metadata", error: documentError });
    redirect(`/${locale}/shipments/${shipmentId}?message=${encodeURIComponent(genericActionError(locale))}`);
  }
  ...
  await supabase.from("shipment_audit_log").insert({
    ...
    shipment_id: shipment.id,
    actor_user_id: user.id,
    action: "bol_draft_generated",
    metadata_json: { storage_bucket: GENERATED_BUCKET },
  });

  revalidatePath(`/${locale}/shipments/${shipmentId}`);
  revalidatePath(`/${locale}/documents`);
  redirect(`/${locale}/shipments/${shipmentId}?message=${encodeURIComponent("BOL brouillon généré en privé.")}`);
}
