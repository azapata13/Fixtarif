...
export async function GET(request: NextRequest, { params }: { params: Promise<{ locale: string }> }) {
  ...
  if (!code) {
    return NextResponse.redirect(new URL(`/${locale}/login?message=Connexion%20Google%20annul%C3%A9e.`, requestUrl.origin));
  }
  ...
  if (error) {
    ...
    return NextResponse.redirect(new URL(`/${locale}/login?message=${encodeURIComponent(genericOAuthError(locale))}`, requestUrl.origin));
  }
